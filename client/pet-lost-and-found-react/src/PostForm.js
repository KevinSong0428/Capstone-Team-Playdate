...
function PostForm() {
        ...
        try {
            const response = await fetch(
                post.animal.animalId
                    ? `${animalUrl}/${post.animal.animalId}`
                    : animalUrl,
                init
            );
            if (!response.ok) {
                throw new Error(
                    `Failed to ${post.animal.animalId ? "update" : "create"} animal: ${response.status
                    }`
                );
            }
            ...
            console.error(
                `Error ${post.animal.animalId ? "updating" : "creating"} animal: `,
                error
            );
        }
    };
    ...
    const addUser = async () => {
        ...
        try {
            const response = await fetch(
                post.user.userId ? `${userUrl}/${post.user.userId}` : userUrl,
                init
            );

            if (!response.ok) {
                throw new Error(
                    `Failed to ${post.user.userId ? "update" : "create"} user: ${response.status
                    }`
                );
            }
            ...
            console.error(
                `Error ${post.user.userId ? "updating" : "creating"} user: `,
                error
            );
        }
    };
        ...
        try {
            const response = await fetch(
                post.location.locationId
                    ? `${locationUrl}/${post.location.locationId}`
                    : locationUrl,
                init
            );
            ...
            if (!response.ok) {
                throw new Error(
                    `Failed to ${post.location.locationId ? "update" : "create"
            ...
            console.error(
                `Error ${post.location.locationId ? "updating" : "creating"
                } location: `,
                error
            );
        }
            ...
            <main className='container'>
                ...
                <form onSubmit={handleSubmit}>
                    <fieldset>
                        ...
                        <div className='form-group'>
                            <label htmlFor='animal.characteristic'>
